...
val projectName = "cactus"
val projectVersion = "1.2.1"

val projectMainClass = "com.neo.sk.cactus.Boot"
...
lazy val backend = (project in file("backend")).enablePlugins(PackPlugin)
  ...
  .settings(name := "cactus")
  .settings(
    //pack
    // If you need to specify main classes manually, use packSettings and packMain
    //packSettings,
    // [Optional] Creating `hello` command that calls org.mydomain.Hello#main(Array[String])
    packMain := Map("cactus" -> projectMainClass),
    packJvmOpts := Map("cactus" -> Seq("-Xmx256m", "-Xms64m")),
    packExtraClasspath := Map("cactus" -> Seq("."))
  )
  .settings(
